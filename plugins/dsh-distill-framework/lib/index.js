...
function buildSkillContent(root) {
    ...
    if (root) {
        try {
            ...
            const dispose = ctx.skills.register({
                name: 'mbdf-flv',
                description: '把一组同领域的经典书(N 本,N≥3)蒸馏成可被 Agent 调用的 skill 工具包。触发: 用户说"把 X 领域的 N 本书蒸馏成 skill"/"用多书融合方法蒸馏"/"多书融合拆书"等。不适用: 单本书(用 cangjie-skill)、纯文学、读后感、作者人设。',
            ...
            ctx.logger.warn(`dsh-distill-framework: failed to register mbdf-flv: ${e.message}`);
        }
    }
    else {
