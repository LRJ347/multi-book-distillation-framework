...
export function apply(ctx) {
    ...
    const SKILL_ROOT = process.env.DSH_AGENT_SKILL_DIR ??
        pickExisting([
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            try {
                ...
                ctx.logger.warn(`dsh-agent-skills: failed to register ${skillName}: ${e.message}`);
            }
        }
        ctx.logger.info(`dsh-agent-skills: registered ${registered} skills from ${SKILL_ROOT}`);
    }
    catch (e) {
        ctx.logger.warn(`dsh-agent-skills: cannot read ${SKILL_ROOT}: ${e.message}`);
    }
    return () => {
